...
\begin{enumerate}
  ...
  \item[] \emph{Geração de chaves.} Calcule o valor $k = max(\{h_i + \lceil \log_2 t_i \rceil : 0 \leq i \leq d - 1\})$, e gere máscaras de bits aleatórias, reutilizadas em cada camada da árvore: $\mathbf{r} = (r_0, \dots, r_{k - 1}) \stackrel{\$}{\longleftarrow} \binwds{m}$. Sementes para cada camada da hiper-árvore também são escolhidas desta maneira: $\textbf{s} = (s_0, \dots, s_{d - 1}) \stackrel{\$}{\longleftarrow} \binwds{m}$, e estados do algoritmo BDS são inicializados: $\textsc{State} = (\textsc{State}_{\textsc{BDS}}^{0}, \dots, \textsc{State}_{\textsc{BDS}}^{d - 1})$. Utilizando instâncias do algoritmo \textsc{Treehash} e a tupla $\textbf{s}$ a fim de criar os pares de chaves de \textsc{Wots+}, os valores $T_{h_{i}, 0}, \; 0 \leq i \leq d - 2$ são calculados e assinados pela primeira folha da árvore superior. Estas assinaturas são então extraídas como $\Sigma = (\sigma_{1}^{T}, \dots, \sigma_{d - 1}^{T})$. Note que apenas as assinaturas das primeiras raízes à esquerda são armazenadas, visto que será a primeira árvore a ser utilizada para assinar mensagens. Para facilitar a construção das árvores irmãs em camadas interiores, sementes pseudoaleatórias, estados do algoritmo BDS e instâncias do algoritmo \textsc{Treehash} são também armazenados em uma estrutura $\textsc{Next} = (s^{n}_{0}, s^{)$
  
  a fim de calcular o valor $T_{h_{d - 1}, 0}$.
...
\todo[inline]{visualização de multiárvores}
...
Por fim, é possível notar que todos os esquemas baseados em árvores de Merkle discutidos apresentam gerenciamento de estado. Esquemas mais complexos, como a família SPHINCS~\cite{Bernstein2015,sphincsplus}, utilizam várias estratégias combinadas a fim de evitar esta característica: a altura da hiper-árvore aumenta substancialmente o número de assinaturas que podem ser feitas com apenas um par de chaves; nos nodos folha mais inferiores, o esquema HORST é utilizado para evitar colisões entre assinaturas; o índice de folha escolhido depende de um valor pseudoaleatório; entre outros recursos. Entretanto, as otimizações descritas na próxima seção têm aplicação reduzida a estes esquemas, visto que o cerne do processamento total destes esquemas não encontra-se no cálculo de assinaturas entre árvores intermediárias.
