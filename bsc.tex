...
são chamados de nodos cauda (\emph{tail nodes}). O algoritmo~\ref{alg:3} demonstra
a versão completa de \textsc{Treehash}; note que uma versão simplificada pode ser
considerada para folhas separadas 
...
\begin{algorithm}[ht]
  ...
  \For{$i \leftarrow 0$ \KwTo $2^{H} - 1$}{
    $n_1 \longleftarrow \textsc{Leafcalc}(i)$ \\
    \While{$n_1.height() == \mathbf{S}.top().height()$}{
      $n_2 \longleftarrow \mathbf{S}.pop()$ \\
      $n_1 \longleftarrow \hash{n_2 \concat n_1}$
    }
    $\mathbf{S}.push(n_1)$
  }
