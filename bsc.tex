...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[scale=0.5]
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Representação gráfica da construção
...
busca resolver este problema através da substituição de $f$ por uma computação
baseada na publicação de um subconjunto como a assinatura, escolhido de
acordo com a mensagem desejada, a partir de uma lista de inteiros trivial.
...
todos os resumos possíveis. Seja o conjunto sequencial $T = \{0, \dots, t - 1\}$,
e a função de mão única $f : \binwds{m} \longrightarrow \binwds{m}$.
A função $\fhash{m}$ utilizada deve apresentar também a característica de
...
$r + 1$ mensagens tal que $\hash{m_{r + 1}} \subseteq \bigcup^{r}_{i = 1} \hash{m_i}$.
A escola de $r$ não é diretamente discutida, visto que acredita-se que uma função de resumo criptográfico usual provê esta característica para valores suficientemente pequenos.
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \node[draw, minimum width=3em, minimum height=1.75em] (s0) at (0.5, -1) {$\sigma_0$};
        ...
        \node[draw, minimum width=3em, minimum height=1.75em, right = 7.5pt of s2] (s3) {$d_{i}$};
        \node[minimum width=1.5em, minimum height=1.75em, right = 2pt of s3] (s4) {$\dots$};
        ...
        \draw[-latex] (d3) -- (x2);
    \end{tikzpicture}
    \caption{Representação gráfica do mapeamento de valores do resumo $d$ para valores de $\sk{}$, para $0 \leq i \leq k - 1, \; 0 \leq j, l \leq t - 1$, no HORS.}
    \label{fig:7}
\end{figure}

É possível visualizar este processo na Figura~\ref{fig:7}. Note que uma conversão de palavras binárias para inteiros está implícita no diagrama, e que elementos $d_i$ podem ter valores iguais, e.g. $(d_{0})_{10} = (d_{i-1})_{10} = l$.
\todo[inline]{just a few more? HORST?}
