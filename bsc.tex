...
\begin{enumerate}
  ...
  \item[] \emph{Geração de chaves.} Tome uma semente $s \stackrel{\$}{\longleftarrow} \binwds{m}$ e uma função de resumo $h_k \stackrel{\$}{\longleftarrow} \hh{}_m$. Gere o conjunto de máscaras $\mathbf{r} = (r_0, \dots, r_{h + \lceil \log_{2} t \rceil - 1}) \stackrel{\$}{\longleftarrow} \binwds{2m}$. Para gerar as chaves privadas das instâncias de esquemas de assinatura única, defina $\textsc{Ots}_{\sk{}}^{i} = \textsc{Gen}_t(f_s(i)), \forall i \in \{0, \dots, 2^h - 1\}$. A árvore de Merkle $T$ é construída utilizando a compressão de $\textsc{Ots}_{\pk{}}^{i}$ explicada, bem como o processo usual para o cálculo de resumos intermediários. Finalmente, a chave privada deve guardar o estado do esquema, dado que esta informação é sensível, porém crucial para o gerenciamento deste. Como o estado pode ser representado como o índice da primeira folha não utilizada, $\sk{} = (s, 0)$. Naturalmente, $\pk{} = T_{0,0}$.
