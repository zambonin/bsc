...
altura $H \in \mathbb{N}^{*}\setminus\{1\}$, onde um nodo $T_{h,j}, 0 \leq j \leq 2^{H - h} - 1, h \in \{0, \dots, H\}$ (o $j$-ésimo à esquerda na altura $h$), e uma função de resumo criptográfico $\fhash{m}$. As folhas são
construídas a partir do resumo de qualquer dado $\mathcal{D}$, e seus nós
intermediários a partir das folhas:
\begin{equation}
  \begin{split}
    T_{0, j} &= \hash{\mathcal{D}_j}, 0 \leq k \leq 2^{h} - 1, \text{ e } \\
