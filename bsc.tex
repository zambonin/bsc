...
\begin{enumerate}
  ...
  \item[] \emph{Verificação da assinatura.} Para verificar uma assinatura perante $T$, assegure que $\textsc{Ots}_{\mathcal{V}}(\textsc{Ots}_{\pk{}}^{j}, \textsc{Ots}_{\sigma}^{j}) = 1$, obtendo $\textsc{Ots}_{\pk{}}^{j}$. Com esta chave, reconstrua a árvore-$l$ com as máscaras $\mathbb{r}$
