...
baseada na publicação de um subconjunto como a assinatura, escolhido a partir
da mensagem desejada, a partir de uma lista de inteiros trivial.
...
trivial $T = \{0, \dots, t - 1\}$, e a função de mão única $f : \binwds{m}
\longrightarrow \binwds{m}$. A função $\fhash{m}$ utilizada deve ser também
\emph{$r$-subset-resilient}, ou seja, deve ser computacionalmente inviável
para um atacante encontrar $r + 1$ mensagens tal que $\hash{m_{r + 1}} \subseteq \bigcup$
