...
Em ambientes onde é desejável que a verificação de assinaturas seja acelerada,
mas a geração destas é rara, ou realizada em um dispositivo com poder de
processamento amplificado

A aplicação de uma
função de resumo criptográfico à mensagem desejada decide 
% This becomes interesting in settings where there are hard performance
% requirements for some steps but not for others. E.g., if you need realtime
% verification with small signatures, but do not care about signing speed at all,
% you want to set w pretty high and then use your trade-off to shift computation
% time to the signature generation.
