...
baratas.}, $0 \leq i, j < 4$, a partir de uma chave $K$ de tamanho $n \in \{128, 192,
...
recursão
$$RC_0 = x^0, \quad RC_1 = x^1, \quad RC_j = x^{j-1} \cdot RC_{j-1}, \; j > 2.$$
...
criando estruturas com várias funcionalidades diferentes. Funções de
...
espalhamento. Por outro lado, é imposto para um função de mão única que
o cálculo de $\hh{}^{-1}$ seja computacionalmente inviável, i.e. o item (iv),
Por fim, funções resistentes à segunda pré-imagem e/ou colisões são definidas
como \emph{funções de resumo criptográfico}, adequadas para utilização no
...
uma relação aparente entre entradas e saídas da função, tratando-se do
resumo por completo e também de subseções deste.  Outra característica desejada é o
...
Neste capítulo, são apresentados vários esquemas de assinatura digital única
começando pelo original, Lamport, a fim de fundamentar o raciocínio por trás
do esquema alvo. Winternitz e \textsc{Wots+} são definidos logo após. Esquemas
convencionais baseados em árvores de Merkle, como o homônimo, XMSS
(\emph{eXtended Merkle Signature Scheme}) e sua variante multiárvores
XMSS$^{MT}$, e SPHINCS são abordados logo após. Em especial, estes
