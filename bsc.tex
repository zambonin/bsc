...
Formalmente, seja uma árvore binária perfeita $T$ de altura $h \in \mathbb{N}, h \geq 2$, cujos nós são denotados como $T_{i,j}, 0 \leq i \leq h - 1, 0 \leq j < 2^{i}$, e defina $T_{0,0}$ como a raiz desta. Tome um esquema de assinatura digital única genérico \textsc{Ots}, seu par de chaves como $\textsc{Ots}_\text{\sk{}}$ e $\textsc{Ots}_\text{\pk{}}$, a assinatura criada por este esquema como $\textsc{Ots}_\sigma$, e os algoritmos de geração de chaves, 
