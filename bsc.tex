...
\begin{algorithm}[ht]
  \KwData{$H$, a altura da árvore $T$, $K$, um parâmetro de compensação, $s, \; 0 \leq s \leq 2^{H} - 2$, índice de uma folha, $\textsc{State}_{\textsc{BDS}}$, o estado do algoritmo}
  \KwResult{$\textsc{Auth}$, o caminho de autenticação para a folha $s + 1$}
\vspace{2mm}
  $\tau \longleftarrow max(h, \sfrac{2^h}{s + 1})$\;
  \If{}
  \ifthenelse{$\lfloor \sfrac{s}{2^{\tau + 1}} \rfloor$ \And $\tau < H - 1$}{}{}
