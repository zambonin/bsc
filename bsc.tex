...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \draw[-latex] (d3) -- (x2);
    \end{tikzpicture}
    \caption{Representação gráfica do mapeamento de valores do resumo $d$ para valores de $\sk{}$, para $0 \leq i \leq k - 1, \; 0 \leq j, l \leq t - 1$, no HORS. Note que uma conversão de palavras binárias para inteiros está implícita no diagrama, e que elementos $d_i$ podem ter valores iguais, e.g. $(d_{0})_{10} = (d_{i-1})_{10} = l$.}
    \label{fig:my_label}
\end{figure}
