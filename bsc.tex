...
\begin{enumerate}
    ...
    \item[] \emph{Geração de chaves.} Defina $\text{\sk{}} = \big( \begin{smallmatrix}
    x_{0, m-1} & x_{0, m - 2} & \dots & x_{0,0} \\
    x_{1, m-1} & x_{1, m - 2} & \dots & x_{1,0}
    \end{smallmatrix} \big)$
    
    $\text{\sk{}} = (x_{0, m-1}, x_{1, m-1}, \dots, x_{0,0}, x_{1,0}) \stackrel{\$}{\longleftarrow} \binwds{m}$ como a chave privada, informalmente uma $2m$-tupla de inteiros pseudoaleatórios. A chave pública $\text{\pk{}} = (y_{0, m-1}, y_{1, m-1}, \dots, y_{0,0}, y_{1,0})$ é derivada de \sk{}, computando $f(x) \; \forall x \in  \text{\sk{}}$. Logo, $\text{\pk{}} = (f(x_{0, m-1}), f(x_{1, m-1}), \dots, f(x_{0,0}), f(x_{1,0}))$.
