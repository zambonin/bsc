...
definição de novos esquemas variantes. Em especial, o esquema proposto
em~\cite{cruz2016constant} torna a complexidade deste passo previsível
através de uma função que mapeia a mensagem para uma tupla com propriedades
especiais; em~\cite{Steinwandt2008}, a mensagem é modificada a fim de gerar
um resumo criptográfico cuja codificação \emph{run-length} é otimizada;
e em~\cite{mcgrew-hash-sigs-11}
...
que retorna o conteúdo da $k$-ésima folha, \emph{i.e.} $\hash{\textsc{Ots}^{k}_{\pk{}}}$,
...
\begin{figure}[ht]
    ...
    \end{tikzpicture}
    \label{fig:13}
    \caption{Valores de $R$ para que modificações de $\mathcal{B}_{1}^{i}$ sejam efetivas.}
\end{figure}
...
& - &    \textsc{Wots} &  523438.00 &   32839.56 &  556277.56 \\
 ...
 & 3500 &   &  261912.93 &   35384.39 &  297297.32 \\
    ...
    \end{tabular}
    \label{tab:3}
    \caption{Iterações de $f$ na etapa de geraç£o de assinaturas ao minimizar $\mathcal{B}_1$.}
\end{table}

Considerando a maximizaÃ§Ã£o de $\mu(\mathcal{B}_1)$, Ã© possÃ­vel perceber uma reduÃ§Ã£o de aproximadamente $2^w$ iteraÃ§Ãµes de $f$ para o esquema \textsc{Wots-b}, condizente com o nÃºmero de \emph{bits} inutilizados apresentados na Tabela~\ref{tab:1}. No caso de \textsc{Wots-r}, observam-se reduÃ§Ãµes de atÃ© $25\%$ para $w = 4$, $33\%$ para $w = 8$ e $42\%$ para $w = 16$. Aplicando ambas as otimizaÃ§Ãµes, o esquema \textsc{Wots-br} resulta em melhorias de atÃ© $28\%$, $39\%$ e $52\%$, respectivamente. No caso da minimizaÃ§Ã£o, tÃªm-se refinamentos de $28\%$, $35\%$ e $47\%$.

Note que a reduÃ§Ã£o no cÃ´mputo destas iteraÃ§Ãµes Ã© diretamente traduzida na etapa complementar, ou seja, na maximizaÃ§Ã£o de $\mu(\mathcal{B}_1)$, existirÃ¡ um aumento do nÃºmero de iteraÃ§Ãµes na geraÃ§Ã£o da assinatura, proporcional Ã  otimizaÃ§Ã£o obtida no passo de verificaÃ§Ã£o. O mesmo conceito aplica-se para a minimizaÃ§Ã£o. Ademais, Ã© preciso considerar a busca de $\lambda_k$ junto aos custos da geraÃ§Ã£o da assinatura. Em vista disso, valores de $R$ muito altos em relaÃ§Ã£o ao parÃ¢metro $w$ sÃ£o indesejÃ¡veis quando este Ã© reduzido, \emph{e.g.} com $w = 4$, o valor $R = 200$ torna-se uma fraÃ§Ã£o substancial do custo total para a geraÃ§Ã£o da assinatura. Ainda assim, casos de uso onde mostra-se necessÃ¡rio otimizar completamente o passo de verificaÃ§Ã£o podem fazer uso de valores excedentes para $R$.

\subsection{Impacto em esquemas baseados em Ã¡rvores de Merkle}
\label{subsection:impact}

Evidentemente, as otimizaÃ§Ãµes propostas nos esquemas \textsc{Wots-b} e \textsc{Wots-r} podem ser aplicadas a quaisquer variantes da famÃ­lia Winternitz. Consequentemente, qualquer esquema baseado em Ã¡rvores de Merkle pode fazer uso de instÃ¢ncias otimizadas destes em suas folhas. A fim de demonstrar esta afirmaÃ§Ã£o, a implementaÃ§Ã£o\footnote{\url{https://github.com/zambonin/xmss-reference}} de \wotsplus{}, esquema de assinatura Ãºnica escolhido para a variante de \xmss{} definida em~\cite{irtf-cfrg-xmss-hash-based-signatures-12} e explorada na SubseÃ§Ã£o~\ref{subsection:wotsplus}, Ã© modificada de acordo.

A Tabela~\ref{tab:4} apresenta os resultados obtidos, na forma da mÃ©dia do tempo de execuÃ§Ã£o de $2^{14}$ instÃ¢ncias dos esquemas \xmss{}, \xmssb{}, \xmssr{} e \xmssbr{}, de acordo com a notaÃ§Ã£o fixada nas seÃ§Ãµes~\ref{section:wotsb} e~\ref{section:wotsr}. Note que $w = 16$ nÃ£o Ã© considerado visto que, para este parÃ¢metro, a etapa de geraÃ§Ã£o de chaves para o esquema \xmss{} torna-se demasiado prolongada. Novamente, as otimizaÃ§Ãµes da soma de verificaÃ§Ã£o nÃ£o sÃ£o consideradas quando deseja-se otimizar a geraÃ§Ã£o de assinaturas.

Os experimentos foram realizados utilizando um computador com as seguintes especificaÃ§Ãµes: 8 GB de RAM DDR3 @ 1333MHz, Intel Core i5-4570 @ 3.2GHz e \texttt{gcc} 7.3.0. Neste ambiente, Ã© possÃ­vel observar que qualquer valor de $R$ melhora o tempo de verificaÃ§Ã£o ao aplicar a maximizaÃ§Ã£o de $\mu(\mathcal{B}_1)$ e otimizaÃ§Ã£o da soma de verificaÃ§Ã£o, obtendo melhoras de atÃ© $22\%$ com $w = 4$ e $36\%$ para $w = 8$. Entretanto, a escolha de valores muito altos para $R$ na aplicaÃ§Ã£o da minimizaÃ§Ã£o de $\mu(\mathcal{B}_1)$ torna o passo de geraÃ§Ã£o de assinatura mais lento. Assim, recomenda-se $R \leq 25$ para $w = 4$, e $R \leq 200$ para $w = 8$.
...
\begin{table}[htbp]
    ...
    \end{tabular}
    \label{tab:4}
    \caption{Tempo de execuÃ§Ã£o, em ms, para as otimizaÃ§Ãµes integradas no \xmss{}.}
\end{table}

\chapter{ConclusÃ£o}
\label{chapter:conclusion}

Neste trabalho, esquemas de assinatura digital baseados em funÃ§Ãµes de resumo criptogrÃ¡fico foram discutidos, a fim de prover um embasamento teÃ³rico para o desenvolvimento de otimizaÃ§Ãµes sobre estes esquemas. VÃ¡rios conceitos relevantes para a Ã¡rea sÃ£o apresentados, como a estrutura de dados chamada de Ã¡rvore de Merkle, que habilita a criaÃ§Ã£o de esquemas de assinatura digital convencionais com mÃºltiplas instÃ¢ncias de assinaturas Ãºnicas, o encadeamento de Ã¡rvores a fim de aumentar o nÃºmero de mensagens passÃ­veis de assinatura, e o gerenciamento sobre estas Ã¡rvores quando seu armazenamento total torna-se impraticÃ¡vel.

O resultado principal, consistindo na otimizaÃ§Ã£o da famÃ­lia de esquemas de assinatura digital Ãºnica Winternitz, introduz um parÃ¢metro de compensaÃ§Ã£o que habilita o deslocamento do cÃ´mputo de iteraÃ§Ãµes de $f$ da geraÃ§Ã£o da assinatura para a verificaÃ§Ã£o, e vice-versa. Para fundamentar este trabalho, experimentos foram realizados em esquemas do estado da arte, com resultados positivos.

\section{Trabalhos futuros}
\label{section:future}

A otimizaÃ§Ã£o apresentada na SeÃ§Ã£o~\ref{section:wotsr} pode considerar a modificaÃ§Ã£o da tupla $\mathcal{B}_1$ de acordo com outros critÃ©rios, adicionais Ã  maximizaÃ§Ã£o ou minimizaÃ§Ã£o de seus elementos, ou substituindo este processo completamente. Por exemplo, visto que o cÃ´mputo de iteraÃ§Ãµes de $f$ Ã© feito sequencialmente, a busca de um critÃ©rio a fim de obter elementos $b \in \mathcal{B}_1$ prÃ³ximos em magnitude habilita o processamento paralelo destas iteraÃ§Ãµes, e portanto um tempo de execuÃ§Ã£o reduzido para a geraÃ§Ã£o e verificaÃ§Ã£o de assinaturas.

Experimentos que exploram as propostas do CapÃ­tulo~\ref{chapter:tuning} podem ser realizados em variantes diferentes da famÃ­lia Winternitz, como \wotsprf{} e \wotst{}, bem como em outros esquemas baseados em Ã¡rvores de Merkle. Ademais, para variantes que necessitam apenas de resistÃªncia Ã  segunda prÃ©-imagem, a seguranÃ§a do esquema \wotsr{} deve ser formalizada, visto que a modificaÃ§Ã£o de $\mathcal{B}_{1}$ pode introduzir imperfeiÃ§Ãµes nas cadeias de resumos subsequentes.
