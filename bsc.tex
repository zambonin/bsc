...
\begin{equation}
    \begin{split}
        RC_0 = x^0 \\
        RC_1 = x^1 \\
        RC_j = x^{j-1} \cdot RC_{j-1}, j > 2        
    \end{split}
\end{equation}


$$. Inicialmente, $\{k_0, \dots, k_{\ell - 1}\} = K$, e $\forall i \geq \ell, K^{e}_{i} = K^{e}_{i - \ell} \oplus \mathbf{k}$. A palavra $\mathbf{k}$, inicialmente com valor $K^{e}_{i - 1}$, pode ser modificada de acordo com as restrições (mutualmente exclusivas) abaixo.
\begin{equation}
    \mathbf{k} = 
        \begin{cases}
