...
\usepackage{
  ...
  etoolbox, multirow, pgfplots, tikz, tikz-qtree, todonotes, xfrac
}
  ...
  \item \textsc{KeyExpansion} consiste da criação de um conjunto de palavras
      $K_e$ de 32 \emph{bits}.  Tome $\ell = \frac{n}{32}$, e assumindo que é
        ...
        \end{equation}\simbolo{$\stackrel{\curvearrowright}{
            \ll{}}$}{Deslocamento circular de \emph{bits}}
  ...
  \item[] \emph{Geração de chaves.} Defina
      \begin{equation}
        \sk{} = (x_{0, m-1}, x_{1, m-1}, \dots, x_{0, 0}, x_{1, 0})
          \random{} \binwds{m}
      \end{equation}
        como a chave privada\simbolo{$\random{}$}{Seleção aleatória},
        informalmente uma $2m$-tupla de inteiros pseudoaleatórios. A chave
        pública $\pk{}$ é derivada de $\sk{}$, computando $f(x) \; \forall x
        \in \sk{}$. Logo,
        \begin{equation}
...
\begin{enumerate}
  ...
  \item[] \emph{Geração de chaves.} Defina
      ...
      \end{equation}
        ...
        \sk{}$\simbolo{$f^{\alpha}$}{Aplicação de $f$ $\alpha$ vezes}.
        Logo,
        \begin{equation}
