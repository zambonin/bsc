...
\begin{align}
    ...
    \end{split}
\end{align}
Então, para garantir que estes eventos aconteçam com altíssima probabilidade, utiliza-se a distribuição binomial a fim de procurar a quantidade mínima de experimentos em que pelo menos um destes produza o valor desejado. Este comportamento pode ser visualizado na Figura~\ref{fig:13}, com quantidades sugeridas de $\{25, 200, 3500\}$ para, respectivamente, parâmetros $w = \{4, 8, 16\}$, recomendados em~\cite{irtf-cfrg-xmss-hash-based-signatures-12}, respectivamente. Note que os valores para as probabilidades acima podem ser modificados a fim de produzir tuplas $\mathcal{B}_{i}^{1}$ cujas médias são maiores, entretanto, $R$ crescerá de modo diretamente proporcional.
