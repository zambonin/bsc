...
\begin{enumerate}
    ...
    Executa-se o algoritmo $\textsc{Ots}_\mathcal{K}$ a
    fim de preencher todas as folhas de $T$. Portanto, $T_{h - 1, j} = \mathcal{H}(\textsc{Ots}_\text{\pk{}}), 0 \leq j < 2^{h - 1}$, e $T$ é construída
    recursivamente. Finalmente, $\text{\pk{}} = $
