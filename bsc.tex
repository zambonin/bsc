...
Encontra-se em processo de padronização pela IETF (\emph{Internet Engineering Task
Force}) um esquema que combate este tipo de ataque, chamado \textsc{Wots-lm}, publicado
originalmente como uma patente estadunidense em 1995 (cite) --- desta maneira
...
autores (cite) buscam introduzir identificadores únicos para cada instância do
esquema, modificando essencialmente a estrutura da função $f$, assim combatendo
ataques multi-usuário. Adicionalmente, o esquema \textsc{Wots-t} (cite) também
foca em combater 
...
\todo[inline]{this should be actually easy to explain}
