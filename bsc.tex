...
\usepackage{amsmath, amsfonts, chronology, enumitem, glossaries, parskip, tikz, tikz-qtree, xfrac}
...
\begin{abbrv}
    ...
    \item[\textsc{Wots-lm}]  \emph{Leighton and Micali's Winternitz One-time Signature Scheme}
    \item[\textsc{Wots-prf}]  \emph{Winternitz One-time Signature Scheme with Pseudorandom Functions}
    \item[\textsc{Hors}]  \emph{Lamport-Diffie One-time Signature Scheme}
    \item[\textsc{Horst}]  \emph{Lamport-Diffie One-time Signature Scheme}
    \item[\textsc{Mss}]  \emph{Lamport-Diffie One-time Signature Scheme}
    \item[\textsc{Cmss}]  \emph{Lamport-Diffie One-time Signature Scheme}
    \item[\textsc{Gmss}]  \emph{Lamport-Diffie One-time Signature Scheme}
    \item[\textsc{Xmss}]  \emph{Lamport-Diffie One-time Signature Scheme}
    \item[\textsc{Xmss-mt}]  \emph{Lamport-Diffie One-time Signature Scheme}
    \item[\textsc{Xmss-t}]  \emph{Lamport-Diffie One-time Signature Scheme}
    \item[\textsc{Sphincs}]  \emph{Lamport-Diffie One-time Signature Scheme}
\end{abbrv}
...
Observe que, por conta de sua semelhança com o esquema SPR-MSS, a estrutura da árvore de Merkle é modificada. A função de construç£o da Ã¡rvore Ã© remodelada a fim de utilizar mÃ¡scaras de \emph{bits} $r_i \stackrel{\$}{\longleftarrow} \binwds{2m}$ para cada nÃ­vel da Ã¡rvore, ou seja,
\begin{equation}
...
\begin{enumerate}
  ...
  \item[] \emph{GeraÃ§Ã£o de chaves.} Tome uma semente $s \stackrel{\$}{\longleftarrow} \binwds{m}$ e uma funÃ§Ã£o de resumo $h_k \stackrel{\$}{\longleftarrow} \hh{}_m$. Gere o conjunto de mÃ¡scaras $\mathbf{r} = (r_0, \dots, r_{H + \lceil \log_{2} t \rceil - 1}) \stackrel{\$}{\longleftarrow} \binwds{2m}$. Para gerar as chaves privadas das instÃ¢ncias de esquemas de assinatura Ãºnica, defina $\textsc{Ots}_{\sk{}}^{i} = \textsc{Gen}_t(f_s(i)), \forall i \in \{0, \dots, 2^H - 1\}$. A Ã¡rvore $T$ Ã© construÃ­da utilizando a compressÃ£o de $\textsc{Ots}_{\pk{}}^{i}$ supracitada, bem como o processo usual para o cÃ¡lculo de resumos intermediÃ¡rios. Finalmente, a chave privada deve guardar o estado do esquema, que pode ser representado como o Ã­ndice da primeira folha nÃ£o utilizada. Portanto, $\sk{} = (s, 0)$, e naturalmente, a chave pÃºblica deve conter as mÃ¡scaras utilizadas em $T$, i.e. $\pk{} = (T_{H,0}, \mathbf{r})$.
  
  \item[] \emph{GeraÃ§Ã£o da assinatura.} A assinatura Ã© construÃ­da de maneira muito similar a esquemas de Merkle clÃ¡ssicos. Para uma mensagem $M$, um resumo $d = h_k(M)$, e um índice $j$
...
\begin{figure}[ht]
    \centering
      \begin{chronology}[4]{1998}{2018}{\textwidth}
        ...
        \event{2007}{GMSS}
        \event{2008}{SPR-MSS}
        \event{2011}{WOTS-PRF, XMSS}
        \event{2012}{XMSS+}
        \event{2013}{XMSS$^{MT}$, WOTS+, WOTS-LM}
