...
As listas são munidas da operação $remove(\cdot)$, que remove o elemento no índice do argumento, desde que este seja válido; as instâncias $\textsc{Treehash}_{h}$ são inicializadas por $init(\cdot)$, cujo argumento é um índice de folha, executadas por $update()$, e munidas da função $height()$, que 
