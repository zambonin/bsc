...
Esquemas de assinatura única (\emph{one-time signature schemes}) são construções
fundamentais para a criação de esquemas baseados apenas em funções de resumo
criptográfico. Estes possibilitam a assinatura de apenas uma mensagem, ao custo de facilitar a falsificação desta no caso da reutilização do
par de chaves. Vários esquemas continuam utilizando assinatura única e compõem o estado da arte da literatura~\cite{sphincsplus,irtf-cfrg-xmss-hash-based-signatures-12}, demonstrando a utilidade da assinatura única e otimizações aplicadas sobre estes algoritmos.
