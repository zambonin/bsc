...
a forja de assinaturas caso esta seja mantida. No caso de Lamport, metade
dos valores são diretamente escolhidos como a assinatura, e em Winternitz,
cadeias de resumos são computadas sobre todos os elementos de $sk{}$. Portanto,
é desejável que esta exposição seja reduzida.

Para tal, a ideia de esquemas de ``poucas assinaturas'' (\emph{few-time signature
schemes}), introduzida originalmente por~\cite{Perrig:2001:BOS:501983.501988}
...
por~\cite{Reyzin:2002:BBS:646039.678322} na forma do esquema HORS
...
Assim, tome um parâmetro de segurança $m$ e o parâmetro de Winternitz $w$, a família de funções $\mathcal{F}_k : \{f_k : \binwds{m} \longrightarrow \binwds{m} \mid k \in
\mathcal{K}_m\}$, a altura $h$ para uma árvore de Merkle $T$, uma família de funções de resumo $\hh{}_k : \{h_k : \binwds{2m} \longrightarrow \binwds{m} \mid k \in
...
\begin{equation}
    T_{i, j} = h_k(r_i \oplus (T_{i + 1, 2j} \concat T_{i + 1, 2j + 1})),
      \; 0 \leq j \leq 2^{i} - 1, \; h - 1 \geq i \geq 0.
\end{equation}
O conteúdo das folhas, originalmente o resumo criptográfico simples de $\textsc{Ots}_{\pk{}}$, é substituído por uma árvore chamada de \emph{árvore-l}, construída a partir dos elementos desta chave de forma similar à árvore anterior, mas com máscaras de bits iguais para todas as árvores referentes aos esquemas de assinatura única. Note que o número de elementos de $\textsc{Ots}_{\pk{}}$, o inteiro $t$ em esquemas baseados em Winternitz, pode não ser uma potência de 2, e portanto a árvore-$l$ deve ser balanceada de forma que qualquer nodo sem irmão à direita é movido para um nível mais alto, de forma a tornar-se um irmão à direita de outro nodo. Assim, a árvore-$l$ tem altura total de $\lceil \log_{2} t \rceil$.

\begin{enumerate}

  \item[] \emph{Geração de chaves.} Tome uma semente $s \stackrel{\$}{\longleftarrow} \binwds{m}$ e uma função de resumo $h_k \stackrel{\$}{\longleftarrow} \hh{}_k$
...
\todo[inline]{describe the scheme with figures, talk about rfc variants, allude that we optimize these}
...
\todo[inline]{talk about horst, list lots of parameters, multitree drawings}
\todo[inline]{ They also proposed to cut
the authentication paths before the root and instead include all Merkle tree nodes at level
x
= 6, to avoid repetitions between overlapping authentication paths and further reduce the
signature size. (they = paper do sphincs)}
