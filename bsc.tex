...
É possível visualizar parte deste processo na Figura~\ref{fig:7}. Note que uma conversão de palavras binárias para inteiros está implícita no diagrama, como setas com preenchimento irregular, e que elementos $d_i$ podem ter valores iguais, e.g. $(d_{0})_{10} = (d_{i-1})_{10} = l$. Percebe-se, a partir da descrição dos algoritmos, que o tamanho de $\pk{}$ é proibitivo para aplicações práticas. Uma otimização será discutida futuramente, que torna o crescimento de $\pk{}$ logarítmico em $\log_2 t$.
