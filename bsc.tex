...
\todo[inline]{comentario legal sobre como no lamport escolhe-se entre 2m valores, no winternitz entre 2 to the w - 1 x t, e no hors k valores entre t, ai depois existem iteraçoes de uma f que é geralmente nao inversivel}
...
é desejável que esta exposição seja reduzida. Para tal, a ideia de esquemas de
``poucas assinaturas digitais'' (\emph{few-time signature schemes}), introduzida
principalmente por~\cite{Perrig:2001:BOS:501983.501988} com o esquema \emph{Bins
and Balls}, uma aplicação do problema clássico em probabilidade.

Todos os esquemas discutidos até o momento sofrem do mesmo problema: uma fração 
considerável de $\sk{}$ é exposta no momento da assinatura, e.g. metade no
...
busca resolver este problema através da substituição de $f$ por um algoritmo distinto. Neste caso, o conteúdo da mensagem controla a seleção de elementos
diferentes em uma lista de inteiros trivial, construindo subconjuntos variados.
...
\todo[inline]{comment about statefulness, some random notes about security,
