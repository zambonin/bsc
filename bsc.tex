...
\end{equation}
Para assegurar este comportamento, uma instância de \wots{} foi produzida com os parâmetros $w = 16$, $m = 256$, $\hh{} =$ SHA-256, $\{\lambda_i : 0 \leq i \leq 2^{16} - 1\} \stackrel{\$}{\longleftarrow} \binwds{32}$, e os resumos $\{\hash{\lambda_i} : 0 \leq i \leq 2^{16} - 1 \land \lambda_i \stackrel{\$}{\longleftarrow} \binwds{32}\}$ foram calculados. As médias das tuplas $\mathcal{B}_{1}^{i}$ são apresentadas na Figura~\ref{fig:12}, concretizando os fatos acima.
...
\begin{align}
    ...
    \end{split}
\end{align}
Então, para assegurar que estes eventos aconteçam com altíssima probabilidade, utiliza-se a distribuição binomial a fim de procurar a quantidade mínima de experimentos em que pelo menos um destes produza o valor desejado. Este comportamento pode ser visualizado na Figura~\ref{fig:13}, com valores sugeridos de $\{25, 200, 3500\}$, respectivamente.
