...
adicionar (iii), obtém-se uma função de mão única, onde o cálculo de
$\mathcal{H}^{-1}$ é computacionalmente inviável. Uma função de resumo
de mão única é obtida adicionando a propriedade (iv). Por fim, uma
...
torna-a adequada para utilização no contexto de segurança da informação. 
...
trocar apenas um \emph{bit} da mensagem $m$
deve modificar cerca de metade dos \emph{bits} do resumo, e vice-versa.

Ademais, note que \textsc{Sec}
e \textsc{Col} apresentam uma sutil diferença: na
...
de bloco. Códigos de autenticação de mensagens, por outro lado, geralmente
englobam todas as seis propriedades, pois funções de resumo criptográfico são
utilizadas em sua construção, e a parametrização habilita autenticação da
mensagem junto à integridade já fornecida.


Ao parametrizar uma destas funções, obtém-se uma função de
compressão de mão única -- mas não de resumo, visto que $\length{X}$ é fixo --,
utilizadas para a construção de cifras de bloco.

Adicionalmente, o conceito de funções de mão única com compressão é
destacado. Estas funções são parametrizadas por uma chave $k$
