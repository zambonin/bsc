...
\chapter{Assinatura digital baseada em árvores de Merkle}

Neste capítulo, são apresentados esquemas de assinatura digital convencionais, que abrigam múltiplas assinaturas sob uma única chave pública, baseados em árvores de Merkle. Os esquemas selecionados são o esquema original de Merkle, 
...
\end{tikzpicture}
\caption{Considere $\mathcal{D}_n$ como qualquer informação ou mensagem desejada. Uma árvore de Merkle pode ser construída recursivamente a partir da concatenação dos resumos dos dados contidos nos filhos de um dado nó. Acima, uma representação gráfica de uma árvore com quatro folhasi}
\end{figure}
