...
Seguindo a notação original em~\cite{Reyzin:2002:BBS:646039.678322}, defina
um parâmetro de segurança $m$ e escolha $t, k \in \mathbb{N},
\binom{t}{k} \geq 2^m, t \gg k$. O tamanho dos pares de chaves e das assinaturas
são fortemente relacionados com, respectivamente, $t$ e $k$. Seja o conjunto
sequencial $T = \{0, \dots, t - 1\}$, e a função de mão única $f : \binwds{m}
\longrightarrow \binwds{m}$. A função $\fhash{m}$ utilizada deve apresentar também a característica de
...
\begin{enumerate}
  ...
  \item[] \emph{Geração de chaves.} Defina $X = (x_{t-1}, \dots, x_0)
      ...
      Aplique $f$ em todos os elementos aleatórios, i.e. 
      
      como a chave privada, e
      $\pk{} = (f(x_{t-1}), \dots, f(x_0))$ como a chave pública.
