...
\end{equation}
Para assegurar este comportamento, uma instância de \wots{} foi produzida com os parâmetros $w = 16$, $m = 256$ e $\hh{} =$ SHA-256, \emph{nonces} criptográficos de 32 bits, e os resumos $\{\hash{\lambda_i} : 0 \leq i \leq 2^{16} - 1\}$ foram calculados. As médias das tuplas $\mathcal{B}_{1}^{i}$ são apresentadas na Figura~\ref{fig:12}, concretizando os fatos acima.

Assim, é possível utilizar este resultado para fornecer valores razoáveis para $R$. Aplicando a tabela da distribuição normal padrão com desvio padrão $\sigma$, identificam-se as probabilidades de $\mu(\mathcal{B}_{1}^{i})$ estar localizada após $\mu'$ de acordo com alguns múltiplos de $\sigma$:
\begin{align}
    \begin{split}
    ...
    \end{split}
\end{align}
Então, para assegurar que estes eventos aconteçam com altíssima probabilidade, i.e. 99.9\%,
utiliza-se a distribuição binomial a fim de procurar a quantidade mínima de experimentos em que pelo menos um destes produza o valor desejado. Este comportamento pode ser visualizado na Figura~\ref{fig:13}.

% é possível observar a função distribuição da distribuição binomial e derivar o número de experimentos desejado. Desse modo, a Figura~\ref{fig:13} provê a visualização necessária.
