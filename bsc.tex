...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \draw[-latex] (x5) -- (s5);
    \end{tikzpicture}
    \caption{Representação gráfica da criação de uma assinatura $\sigma$ no HORS, para $0 \leq i \leq k - 1, \; 0 \leq j, l \leq t - 1$.}
    \label{fig:7}
\end{figure}

É possível visualizar parte deste processo na Figura~\ref{fig:7}. Note que uma conversão de palavras binárias para inteiros está implícita no diagrama, como setas com preenchimento irregular, e que elementos $d_i$ podem ter valores iguais, e.g. $(d_{0})_{10} = (d_{i-1})_{10} = l$.
