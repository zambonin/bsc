...
\begin{algorithm}[ht]
  \KwData{$H$, a altura da árvore $T$, $K$, um parâmetro de compensação, $s, \; 0 \leq s \leq 2^{H} - 2$, índice de folha, $\textsc{State}_{\textsc{BDS}}$, o estado do algoritmo}
...
\vspace{2mm}
  ...
  \ElseIf{$\tau > 0$}{
    ...
    \For{$h = 0$ \KwTo $\tau - 1$}{
        \uIf{$h < H - K$}{
            $\textsc{Auth}_{h} \longleftarrow \textsc{Treehash}_{h}.t$\;
        }
        \Else{
  ...
  \For{$i = 0$ \KwTo $\sfrac{H - K}{2}$}{
    $k \longleftarrow min_{j \in \{0, \dots, H - K - 1\}}\{\textsc{Treehash}_{j}.height()\}$\;
    $k \longleftarrow min(\{\textsc{Treehash}_{j} : j \in 0, \dots, H - K - 1\})$
