...
\begin{enumerate}[label=(\roman*)]
    ...
    \item $\textsc{Keep}$, lista de tamanho $H - 1$
\end{enumerate}

, uma lista $\textsc{Keep}$ de tamanho $H - 1$, que armazena nodos úteis para o cálculo de nodos de autenticação à esquerda, $\textsc{Retain} = T_{H - 2, 3}$, uma pilha \textbf{S}, instâncias de $\{\textsc{Treehash}_{h} : h \in 0, \dots, H - 3\}$.
