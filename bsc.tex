...
\begin{enumerate}
    ...
    de pares de chaves diferentes provenientes de \textsc{Ots}. Então, a partir de execuções de $\textsc{Ots}_\mathcal{K}$ para cada folha, tem-se $\text{\sk{}} = ((\textsc{Ots}_{\text{\sk{}}, 2^{h - 1}}, \textsc{Ots}_{\text{\pk{}}, 2^{h - 1}}), \dots, (\textsc{Ots}_{\text{\sk{}}, 0}, \textsc{Ots}_{\text{\pk{}}, 0}))$.
    As folhas de $T$ são preenchidas com as respectivas chaves públicas de \textsc{Ots}. Portanto, $T_{h - 1, j} = \mathcal{H}(\textsc{Ots}_{\text{\pk{}}, j}), 0 \leq j < 2^{h - 1}$, e $T$ é construída
    recursivamente. Finalmente, $\text{\pk{}} = \mathcal{H}(T_{0,0})$.
