...
RSA e ECDSA --- têm sua segurança baseada na dificuldade de se fatorar números
muito grandes ou na obtenção de logaritmos discretos. Este tipo de cômputo pode
ser eficientemente realizado por um computador quântico, utilizando algoritmos
já conhecidos. Deste modo, para manter o ambiente de assinaturas digitais
seguro, é necessário oferecer alternativas pós-quânticas. Este trabalho busca
apresentar esquemas baseados apenas em funções de resumo criptográfico, cuja
segurança é baseada apenas na resistência à colisões da função escolhida, com o
intuito de discutir uma nova otimização para a família de esquemas de
assinatura Winternitz. Particularmente, esta proposta introduz um parâmetro de
...
De acordo com este comportamento, é apresentada uma customização para o esquema
...
publicado~\cite{Perin:inproc:2018:jun}.
...
gerenciamento de múltiplas assinaturas e fundamentando a otimização proposta,
...
\begin{enumerate}
  ...
  \item[] \emph{Geração de chaves.} Defina
      \begin{equation}
        \sk{} = (x_{t - 1}, \dots, x_{0}) \random{} \binwds{m}
      \end{equation}
        como a chave privada, informalmente uma $t$-tupla de inteiros
        pseudoaleatórios. A chave pública $\pk{} = (y_{t - 1}, \dots, y_{0})$
        pode ser derivada de $\sk{}$, computando $f^{2^{w}-1}(x) \; \forall x
        \in \sk{}$\simbolo{$f^{\alpha}$}{Aplicação de $f$ $\alpha$ vezes}.
        Logo,
        \begin{equation}
...
\mathcal{K}_{m}\},\] ou seja, funções de mão única sem compressão, e
...
r_{2^{w} - 1}) \random{} \binwds{m},\] a nova função de iteração $c^{i}_{k}(x,
r)$ é definida recursivamente como
\begin{equation}
  \begin{split}
    c^{0}_{k}(x, r) &= x, \\
...
% TODO mover para Merkle-Damgård
...
\begin{enumerate}
  ...
  \item[] \emph{Geração de chaves.} Defina $X = (x_{t - 1}, \dots, x_{0})
      \random{} \binwds{m}$, e adicione $k$ para obter a chave privada.
        Portanto,
        \begin{equation}
...
Parte deste processo é visualizado na Figura~\ref{fig:7}. Note que uma
...
\begin{figure}
  ...
  \caption{Caminho de autenticação para a folha $j = 3 \cdot 2^{h_{0}} + 5$,
    em uma hiper-árvore com $d = 2, h_{0} = h_{1} = 3$.}\label{fig:8}
\end{figure}
...
K}{2}$ instâncias desta rotina. Finalmente, o estado $\textsc{State}_{\bds{}}$
é composto das seguintes estruturas:
...
é publicado em~\cite{Merkle:inproc:1989:aug}. Resultados teóricos e otimizações
...
\begin{equation}
  \Lambda = (\lambda_{0}, \dots, \lambda_{R - 1})
    \random{} \binwds{m}.
\end{equation}
Na etapa de geração da assinatura, compute todos os resumos
\begin{equation}
  D = \{\hash{M \concat \lambda_{r}} : 0 \leq r \leq R\}
\end{equation}
...
\emph{nonce} mais conveniente para que a geração de assinaturas seja eficiente,
calcule
\begin{equation}
...
\end{equation}
Então, tome $d = \hash{M \concat \lambda_{k}}$ para a execução do esquema. Note
...
\begin{figure}
  ...
  \caption{Histograma normalizado de médias de $\bone^{i}$, com 50
...
\begin{align}
  ...
  \end{split}
\end{align}
Então, para garantir que estes eventos aconteçam com altíssima probabilidade, a
distribuição binomial é utilizada a fim de procurar a quantidade mínima de
...
16\}$, recomendados em~\cite[Capítulo 6]{Huelsing:report:2018:may}. Note que os
valores para as probabilidades acima podem ser modificados a fim de produzir
tuplas $\bone^{i}$ cujas médias são maiores, entretanto, $R$ crescerá de modo
...
média aritmética de $2^{14}$ conjuntos $\mathcal{B}$, separados em $\bone{}$ e
$\btwo{}$ a fim de enfatizar o efeito de cada esquema variante. Observe que
$\mu(\btwo{})$ é afetada negativamente por \wotsr{} em virtude da maximização
dos elementos em $\bone{}$. Entretanto, esta diferença não impacta
negativamente os ganhos totais. A utilização de \wotsbr{} mitiga este
comportamento. Para a Tabela~\ref{table:argmin}, a otimização da soma de
verificação não é recomendada, visto que atuará de maneira contraditória em
relação à proposta.
...
\begin{table}
  ...
  \begin{tabular}{cclccc}
    \toprule
    $w$ & $R$
      & \textsc{Esquema} & $\mu(\bone{})$
      & $\mu(\btwo{})$ & $\mu(\mathcal{B})$ \\
    \toprule
...
Ademais, é preciso considerar a busca de $\lambda_{k}$ junto aos custos da
...
esquema \xmss{} é demasiado prolongada. Novamente, as otimizações da soma de
verificação não são consideradas quando é desejável otimizar a geração de
assinaturas.
...
\bone{}$ próximos em magnitude habilita o processamento paralelo destas
