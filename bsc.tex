...
$T$ inteiramente. Considere a subrotina $\textsc{Leaf}(k), 0 \leq k \leq 2^{H} - 1$,
...
com as operações $\mathbf{S}.pop()$ e $\mathbf{S}.push(\cdot)$, calcula o nodo
desejado em $2^H$ chamadas de \textsc{Leaf} e $2^{H} - 1$ cálculos de $\hh{}$.
Em qualquer momento de uma execução de \textsc{Treehash}, o número máximo de
nodos armazenados em \textbf{S} será $H$, e os nodos armazenados neste instante
são chamados de nodos de cauda (\emph{tail nodes}).
...
\begin{enumerate}
  ...
  não utilizado anteriormente, calcula-se $\textsc{Ots}_\sigma^{j}$ a partir de $d$. O caminho de autenticaç£o \textsc{Auth} Ã© construÃ­do com o auxÃ­lio de algoritmos de travessia. Assim, $\sigma = (j, \textsc{Ots}_\sigma^{j}, \textsc{Auth})$.
  
  \item[] \emph{VerificaÃ§Ã£o da assinatura.} Para verificar uma assinatura perante $T$, assegure que $\textsc{Ots}_{\mathcal{V}}(\textsc{Ots}_{\pk{}}^{j}, \textsc{Ots}_{\sigma}^{j}) = 1$, e utilize esta chave para reconstruir a Ã¡rvore-$l$ com as mÃ¡scaras $\mathbf{r}$, obtendo a raiz desta. Junto a \textsc{Auth}, compute o caminho de nodos $(a_0, \dots, a_h)$. Finalmente, se $\pk{} = a_{h}$, a assinatura estÃ¡ vÃ¡lida.

\end{enumerate}

O esquema descrito tem estrutura similar Ã  variante em processo de padronizaÃ§Ã£o pela IETF, que faz uso de \textsc{Wots+} em suas folhas. Este esquema tambÃ©m conta com outros arcabouÃ§os que permitem a resistÃªncia a ataques especÃ­ficos, e.g. a associaÃ§Ã£o da assinatura com o usuÃ¡rio e a posiÃ§Ã£o na Ã¡rvore impede ataques que almejam forjar assinaturas para um grande nÃºmero de instÃ¢ncias de esquemas de assinatura Ãºnica~\cite[9.1]{irtf-cfrg-xmss-hash-based-signatures-12}. Entretanto, seu funcionamento geral Ã© similar a esquemas clÃ¡ssicos, e o gerenciamento de estado Ã© mantido. As otimizaÃ§Ãµes apresentadas futuramente sÃ£o implementadas e testadas principalmente neste esquema. 

\subsection{XMSS multiÃ¡rvores}

\todo[inline]{introduce with the idea of the hyper tree in cmss and gmss, follow with xmss mt}

TambÃ©m, como mencionado na seÃ§Ã£o do \textsc{Wots}, o esquema XMSS-T Ã© definido em~\cite{Hulsing:2016:MMA:3081852.3081871}, como uma variante resistente Ã  ataques multiusuÃ¡rio, atribuindo um endereÃ§o Ãºnico a cada nodo na Ã¡rvore, e derivando deste um resumo criptogrÃ¡fico calculado por funÃ§Ãµes com novas noÃ§Ãµes de resistÃªncia Ã  prÃ©-imagem, segunda prÃ©-imagem e colisÃµes, a fim de reduzir as ameaÃ§as ocasionadas por este tipo de ataque.
