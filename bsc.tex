...
\begin{enumerate}
    ...
    \begin{equation}
    \begin{split}
    a_{0} &= \hash{\textsc{Ots}_\text{\pk{}}^{j}} \text{ e, } 1 \leq k < h, \\
    a_{k} &= 
    \begin{cases}
    \hash{a_{k - 1} \concat \textsc{Auth}_{h - k}}, \text{ se } \lfloor \sfrac{j}{2^{k - 1}} \rfloor \equiv 1 \pmod{2} \\
    \hash{p_{k - 1} \concat \textsc{Auth}_{h - k}}, \text{ se } \lfloor \sfrac{j}{2^{k - 1}} \rfloor \equiv 0 \pmod{2} \\
    \end{cases}
    \end{split}
    \end{equation}
