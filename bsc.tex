...
\begin{equation}
    \lim_{n \to \infty} \mu(\{\mu(\mathcal{B}_{1}^{i}) : 0 \leq i \leq n\}) = 2^{w - 1}.
\end{equation}
...
Assim, é possível utilizar este resultado para fornecer valores razoáveis para $R$. Utilizando a tabela da distribuição normal padrão com desvio padrão $\sigma$, identificam-se as probabilidades de $\mu(\mathcal{B}_{1}^{i})$ estar localizada após a média esperada de acordo com alguns múltiplos de $\sigma$:
\begin{align}
    \begin{split}
        P(\mu(\mathcal{B}_{1}^{i}) > 2^{w - 1} + 1\sigma) &= 0.1587, \\
        P(\mu(\mathcal{B}_{1}^{i}) > 2^{w - 1} + 2\sigma) &= 0.0228, \\
        P(\mu(\mathcal{B}_{1}^{i}) > 2^{w - 1} + 3\sigma) &= 0.0013.
    \end{split}
\end{align}
...
\begin{figure}[ht]
    ...
    {1 - (1 - 0.1587)^x};
    \addlegendentry{$P(\mu(\mathcal{B}_{1}^{i}) > 2^{w - 1} + 1\sigma)$}
    \addplot [
        domain=0:200,
        samples=100,
        thick,
    ...
    {1 - (1 - 0.0228)^x};
    \addlegendentry{$P(\mu(\mathcal{B}_{1}^{i}) > 2^{w - 1} + 2\sigma)$}
    \addplot [
        domain=0:200,
        samples=100,
        thick,
    ...
    {1 - (1 - 0.0013)^x};
    \addlegendentry{$P(\mu(\mathcal{B}_{1}^{i}) > 2^{w - 1} + 3\sigma)$}
    \end{axis}
    \end{tikzpicture}
    \label{fig:rplot}
