...
Observe que, por conta de sua semelhança com o esquema SPR-MSS, a estrutura da árvore de Merkle é modificada. A função de construção da árvore é remodelada a fim de utilizar máscaras de \emph{bits} $\mathcal{r}_i \stackrel{\$}{\longleftarrow} \binwds{2m}$ para cada nível da árvore, ou seja,
...
\mathcal{K}_m\}$, a altura da árvore de Merkle $h \in \mathbb{N}$, uma família de funções de resumo $\hh{}_k : \{h_k : \binwds{2m} \longrightarrow \binwds{m} \mid k \in
