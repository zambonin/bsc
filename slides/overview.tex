...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \end{itemize}
    \item For every hash function, there is a correspondent digital signature
      scheme
    \begin{itemize}
...
\begin{frame}
  \frametitle{Foundations -- hash functions}
  \begin{equation*}
...
\begin{frame}
    \frametitle{Foundations -- digital signatures}
  \begin{itemize}
...
\begin{frame}
  ...
  \begin{center}
    \begin{figure}
      \scriptsize
      \begin{chronology}[4]{1979}{2017}{90ex}
        \event[1982]{1979}{LD-OTS, WOTS}
        \event{1982}{MSS}
        \event[1996]{1989}{\footnotesize Theoretical results}
        \event{2002}{HORS}
        \event[2007]{2006}{CMSS, GMSS}
        \event{2011}{XMSS}
        \event{2013}{XMSS$^{MT}$, WOTS+}
        \event{2015}{SPHINCS}
      \end{chronology}
      \captionsetup{width=0.85\textwidth,font=scriptsize,skip=12pt}
      \caption*{Outline of the most important moments in hash-based digital
        signature schemes research, starting in 1979 with Lamport's one-time
        scheme, and shortly after with the work of Merkle, allowing many-time
        schemes through the use of hash trees. \vspace{2mm}

        The years marked as productive for `'theoretical results`' consist
        primarily of research about optimal one-time signature schemes and
...
\begin{frame}
  ...
  Consider $\pad{}$ as a simple zero padding to the left. The hash chain
  exponents $p_i \in \{0, 1\}^w$ are generated as follows:
  \begin{align*}
    \hash{m} \concat 0^* &= (p_{t - 1}, \dots, p_{t - t_1}) \\
    c &= \sum_{i = t - t_1}^{t - 1} (2^w - p_i) \\
    c \concat 0^* &= (p_{t_2 - 1}, \dots, p_{0})
  \end{align*}
...
\begin{frame}
  ...
  \begin{align*}
    ...
    \sigma &= (\mathcal{H}^{p_{t - 1}}(y_{t - 1}), \dots, \mathcal{H}^{p_0}(y_0))
  \end{align*}
  To verify $\sigma$, all $p_i$ are calculated and the hash chains are finished:
  \begin{align*}
    \forall \sigma_i \in \sigma, \hash[2^w - 1 - p_{i}]{\sigma_i} &= P_{k_i}
  \end{align*}
\end{frame}
...
\begin{frame}
  ...
  \begin{figure}[htbp]
    \centering
    \includegraphics[trim={0 0 0 2cm},clip,scale=0.18]{xmss-example}
  \end{figure}
\end{frame}
